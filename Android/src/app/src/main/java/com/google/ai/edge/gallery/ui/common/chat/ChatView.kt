...
fun ChatView(
  ...
  // Local API server toggle state.
  val apiServerViewModel: ApiServerViewModel = hiltViewModel()
  val isApiRunning by apiServerViewModel.isRunning.collectAsState()
  val apiAuthToken by apiServerViewModel.authToken.collectAsState()
  var showApiAuthDialog by remember { mutableStateOf(false) }
  LaunchedEffect(Unit) { apiServerViewModel.loadAuthToken() }
        ...
        Scaffold(
          ...
          topBar = {
            ModelPageAppBar(
              ...
              onApiAuthClicked = { showApiAuthDialog = true },
            )
          },
        ) { innerPadding ->
          Box {
                ...
                when (targetState) {
                  ...
                  true ->
                    ChatPanel(
                      ...
                      onOpenChatHistoryClicked = { scope.launch { drawerState.open() } },
  ...
  // Local API settings dialog (toggle on/off + bearer token).
  if (showApiAuthDialog) {
    var tokenInput by remember(apiAuthToken) { mutableStateOf(apiAuthToken ?: "") }
    AlertDialog(
      onDismissRequest = { showApiAuthDialog = false },
      title = { Text(context.getString(R.string.local_api_settings_title)) },
      text = {
        Column {
          ...
          Row(
            ...
            Text(
              context.getString(R.string.local_api_enable_label),
              style = MaterialTheme.typography.bodyLarge,
              modifier = Modifier.weight(1f),
            )
            Switch(
              checked = isApiRunning,
              onCheckedChange = {
                val result = apiServerViewModel.toggle(model = selectedModel, taskId = task.id)
                scope.launch {
                  when (result) {
                    ApiServerViewModel.STOPPED ->
                      snackbarHostState.showSnackbar(context.getString(R.string.local_api_stopped))
                    null ->
                      snackbarHostState.showSnackbar(
                        context.getString(R.string.local_api_not_started)
                      )
                    else ->
                      snackbarHostState.showSnackbar(
                        context.getString(R.string.local_api_started, result)
                      )
                  }
                }
              },
          ...
          Text(
            context.getString(R.string.local_api_auth_dialog_hint),
          ...
          OutlinedTextField(
            ...
            placeholder = { Text(context.getString(R.string.local_api_auth_placeholder)) },
            singleLine = true,
            modifier = Modifier.fillMaxWidth(),
          )
        }
      ...
      confirmButton = {
        TextButton(
          onClick = {
            apiServerViewModel.setAuthToken(tokenInput)
            showApiAuthDialog = false
          }
        ) {
          Text(context.getString(R.string.local_api_auth_save))
        }
      },
      dismissButton = {
        TextButton(onClick = { showApiAuthDialog = false }) {
