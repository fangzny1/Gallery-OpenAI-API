...
> **⭐ 魔改版 (Fork) — Gallery-OpenAI-API**
>
> 本仓库是 [Google AI Edge Gallery](https://github.com/google-ai-edge/gallery) 的魔改分支,在保留原版全部功能的基础上,**新增了一个 OpenAI 兼容的本地 HTTP API 服务器**,让局域网内的任何 OpenAI 兼容客户端(如 ChatBox、rikkaHub、Python `openai` 库、curl)都能直接调用手机上运行的离线模型。
>
> **新增功能:**
> - 🔌 **OpenAI 兼容 API**:`POST /v1/chat/completions`,支持流式(SSE)与非流式
> - 🔐 **Bearer Token 鉴权**:可选,设置后未带 token 的请求返回 401,校园网/共享网络下更安全
> - 🧵 **完整多轮上下文**:支持跨消息的历史对话
> - 🆕 **新对话自动重置**:客户端新建对话时会话自动清空,不会串上下文
> - 🖼️ **图片理解 (Ask Image)**:支持多模态模型,通过 `image_url`(base64)传图
> - ⏳ **请求排队**:并发请求自动排队,替代原来的 409 报错
> - 📚 **会话历史**:完整对话记录(非仅输入历史),加号菜单新增"会话历史"入口
>
> 详细用法见 [ANDROID_API_README.md](ANDROID_API_README.md)。
